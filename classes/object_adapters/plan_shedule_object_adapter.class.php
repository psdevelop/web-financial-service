...
class PlanSheduleObjectAdapter extends ObjectAdapter implements ObjectManipInterface  {
     ...
     function __construct($table_name, $class_name)    {
        ...
        $this->fields_prev_text_array = array("id"=>"ID","shedule_name"=>"Тип графика платежа");
        $this->fields_width_array = array("id"=>10,"shedule_name"=>25);
        $this->select_display_field = "shedule_name";
        $this->manip_form_template = "<table><tr>
		<td>***___shedule_name</td><td></td><td>***___id</td></tr>
            </table>";
	...
	parent::write_header_td("Тип графика платежа",100);
        echo "</tr>";
     }
     
     function writeTableRow($object, $linked_props)    {
        parent::write_td($object->getId(),25);
	parent::write_td($object->shedule_name,100);
     }
