...
class TinyAjax
{
	...
	public function process() {
		...
		$asize = sizeof($args);
		for($x = 0; $x < $asize; $x++) {
            $args[$x] = str_replace ("%u20AC", "€", ($this->decode($args[$x])));
		}
	 ...
	 */
	public function getStub($func_name)
	{
    	if(!is_array($func_name))
		{
			$html = "function $func_name() {request_uri = \"" . $this->mRequestUri . "\"; aj_call(\"$func_name\", $func_name.arguments);};";
		}
		else
		{
			$params = "";
			$html = "function {$func_name[0]}() {if({$func_name[0]}.arguments.length > 0) {	return aj_call(\"{$func_name[0]}\", {$func_name[0]}.arguments);};";
	...
	private function decode($data) {
        ...
        "ß" => "&szlig;",
        "€" => "&euro;");
        $data = strtr ($data, array_flip($table));
        #return $data;
